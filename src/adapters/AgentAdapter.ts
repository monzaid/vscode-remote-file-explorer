 ...
 */
export class AgentAdapter implements IProtocolAdapter {
  ...
  // P1-3: Security flags
  private agentSecure: boolean = true;
   ...
   */
  private getOrCreateAgents(): { httpAgent: http.Agent; httpsAgent: https.Agent } {
    if (!this.httpAgent) {
  ...
  private validateHost(host: string): void {
    ...
    if (localhostNames.includes(normalizedHost)) {
      if (!this.allowLocalhost) {
        throw new Error(
          'Host validation failed: connection to localhost/loopback is not allowed. ' +
    ...
    if (/^\d+\.\d+\.\d+\.\d+$/.test(normalizedHost)) {
      ...
      if (parts.length === 4 && parts.every(p => p >= 0 && p <= 255)) {
        ...
        if (parts[0] === 10) {
          if (!this.allowLocalhost) {
            throw new Error(
              'Host validation failed: connection to private network (10.0.0.0/8) is not allowed. ' +
              'Set remote-fs.agent.allowLocalhost to true to enable.'
            );
          }
        }
        ...
        if (parts[0] === 172 && parts[1] >= 16 && parts[1] <= 31) {
          if (!this.allowLocalhost) {
            throw new Error(
              'Host validation failed: connection to private network (172.16.0.0/12) is not allowed. ' +
              'Set remote-fs.agent.allowLocalhost to true to enable.'
            );
          }
        }
        ...
        if (parts[0] === 192 && parts[1] === 168) {
          if (!this.allowLocalhost) {
            throw new Error(
              'Host validation failed: connection to private network (192.168.0.0/16) is not allowed. ' +
              'Set remote-fs.agent.allowLocalhost to true to enable.'
            );
          }
        }
        ...
        if (parts[0] === 169 && parts[1] === 254) {
          if (!this.allowLocalhost) {
            throw new Error(
              'Host validation failed: connection to link-local network (169.254.0.0/16) is not allowed. ' +
              'Set remote-fs.agent.allowLocalhost to true to enable.'
            );
          }
        }
    ...
    if (lowerHost.startsWith('[') && lowerHost.endsWith(']')) {
      ...
      if (/^fc/i.test(ipv6) || /^fd/i.test(ipv6)) {
        if (!this.allowLocalhost) {
          throw new Error(
            'Host validation failed: connection to IPv6 ULA (fc00::/7) is not allowed. ' +
            'Set remote-fs.agent.allowLocalhost to true to enable.'
          );
        }
      }
      ...
      if (/^fe[89ab]/i.test(ipv6)) {
        if (!this.allowLocalhost) {
          throw new Error(
            'Host validation failed: connection to IPv6 link-local (fe80::/10) is not allowed. ' +
            'Set remote-fs.agent.allowLocalhost to true to enable.'
          );
        }
      }
  ...
  async connect(config: ConnectionConfig): Promise<void> {
    ...
    this.allowLocalhost = (config as any)['remote-fs.agent.allowLocalhost'] === true;
    ...
    if (config.agentUrl) {
      ...
      const explicitSecure = (config as any).agentSecure;
    ...
    if (!this.agentSecure && this.token) {
      console.warn(
        '[SECURITY WARNING] Agent Token is being transmitted over HTTP (plaintext). ' +
      ...
      const req = httpModule.request(options, (res) => {
        ...
        if (contentLength) {
          ...
          if (!isNaN(len) && len > MAX_RESPONSE_SIZE) {
            ...
            reject(
              new Error(
                `Agent API response Content-Length ${len} exceeds maximum ${MAX_RESPONSE_SIZE} bytes`
              )
            );
            return;
          }
        ...
        res.on('data', (chunk: Buffer) => {
          ...
          if (dataSize > MAX_RESPONSE_SIZE) {
            ...
            reject(
              new Error(
                `Agent API response exceeds maximum size of ${MAX_RESPONSE_SIZE} bytes`
              )
            );
            return;
          }
        ...
        res.on('end', () => {
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            try {
              ...
              reject(
                new Error(
                  `Agent API response processing failed: ${err instanceof Error ? err.message : 'Unknown error'}`
                )
              );
            }
          } else {
            ...
            reject(
              new Error(`Agent API error: ${res.statusCode} ${errorBody}`)
            );
          }
        });
        ...
        res.on('error', (err) => {
          reject(
            new Error(`Agent API response error: ${err.message}`)
          );
        });
      });
   ...
   */
  private mapStat(data: any): RemoteFileStat {
    return {
      type: data.type || 'file',
