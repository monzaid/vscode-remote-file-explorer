...
function getAdapter(connectionId) {
...
function registerAllCommands(_context) {
    const disposables = [];
    // ==================================================================
    // connect
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.connect', async (node) => {
    ...
    // disconnect
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.disconnect', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
        try {
    ...
    // search
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.search', async () => {
        ...
        let results;
        try {
            results = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Searching...', cancellable: true }, async (_progress, token) => {
    ...
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.syncCurrentFile', async (node) => {
    ...
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.uploadCurrentFile', async (node) => {
        let connectionId;
        let remotePath;
        if (node?.connectionId && node?.remotePath) {
            connectionId = node.connectionId;
    ...
    // newFile
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.newFile', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
    ...
    // newFolder
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.newFolder', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
        const adapter = getAdapter(parsed.connectionId);
    ...
    // deleteFile
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.deleteFile', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
        const adapter = getAdapter(parsed.connectionId);
        ...
        const confirm = await vscode.window.showWarningMessage(`Delete ${itemType} "${name}"?`, { modal: true }, 'Delete', 'Cancel');
        if (confirm !== 'Delete')
            return;
        try {
    ...
    // renameFile
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.renameFile', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
        const adapter = getAdapter(parsed.connectionId);
    ...
    // copyPath
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.copyPath', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
    ...
    // downloadToLocal
    // ==================================================================
    disposables.push(vscode.commands.registerCommand('remote-fs.downloadToLocal', async (node) => {
        const parsed = parseNode(node);
        if (!parsed)
            return;
        const adapter = getAdapter(parsed.connectionId);
        ...
        if (!saveUri)
            return;
        try {
            const content = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Downloading ${fileName}...`, cancellable: false }, async () => adapter.readFile(parsed.remotePath));
